...
func BinarySearch(array []int, element int) int {
	...
	*/
	...
	low, high := 0, len(array) // Set the boundaries

	for low < high {
		middle := low + (high-low)/2 // Calculate the middle point every iteration
		...
		if array[middle] == element {
			...
			low = middle // The element is larger than the current value, set the low boundary to middle
		}
	}
